...
#if defined(USE_NIMBLE)
    NimBLEDevice::setSecurityAuth(enable, enable, enable);
    NimBLEDevice::setSecurityIOCap(BLE_HS_IO_NO_INPUT_OUTPUT);
#else
    BLESecurity* pSecurity = new BLESecurity();
    pSecurity->setAuthenticationMode(enable ? ESP_LE_AUTH_REQ_SC_BOND : ESP_LE_AUTH_NO_BOND);
    pSecurity->setCapability(ESP_IO_CAP_NONE);
    pSecurity->setInitEncryptionKey(ESP_BLE_ENC_KEY_MASK | ESP_BLE_ID_KEY_MASK);
#endif
}

void BleKeyboard::setSecurityLevel(uint8_t level) {
#if defined(USE_NIMBLE)
    NimBLEDevice::setSecurityAuth((level & ESP_LE_AUTH_BOND) != 0,
                                (level & ESP_LE_AUTH_REQ_MITM) != 0,
                                (level & ESP_LE_AUTH_REQ_SC_ONLY) != 0);
#else
    BLESecurity* pSecurity = new BLESecurity();
    pSecurity->setAuthenticationMode(level);
#endif
}

void BleKeyboard::clearBondedDevices() {
    esp_ble_remove_bond_device(0); // 0 removes all bonded devices
}
...
void BleKeyboard::begin(void) {
    ...
    // Configurar bonding por defecto
    enableBonding(true);
    setSecurityLevel(ESP_LE_AUTH_REQ_SC_BOND);
...
#if defined(USE_NIMBLE)
    // Configuración adicional para NimBLE
    BLEDevice::setSecurityAuth(true, true, true);
#else
    // Configuración para BLE estándar
    BLESecurity* pSecurity = new BLESecurity();
    ...
    ESP_LOGD(LOG_TAG, "Advertising started! Bonding enabled.");
}
...
void BleKeyboard::onConnect(NimBLEServer* pServer) {
    this->connected = true;
    ESP_LOGI(LOG_TAG, "Client connected (bonded: %d)", NimBLEDevice::getNumBonds());
}

void BleKeyboard::onDisconnect(NimBLEServer* pServer) {
    this->connected = false;
    BLEDevice::startAdvertising();
    ESP_LOGI(LOG_TAG, "Client disconnected - Advertising restarted");
}

void BleKeyboard::onWrite(NimBLECharacteristic* me) {
    ESP_LOGD(LOG_TAG, "Characteristic write event");
}
#else
